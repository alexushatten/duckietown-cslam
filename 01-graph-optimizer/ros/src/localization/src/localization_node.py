...
from duckietown_msgs.msg import (
    BoolStamped, Twist2DStamped, FSMState, AprilTagDetectionArray, WheelsCmdStamped
)
...
class LocalizationNode():
    ...
    def __init__(self):
        ...
        self.tag_ttl = rospy.get_param("/simple_localization/loc_node/tag_ttl")
        # trigger offset measurement procedure on/off
        self.measuring_offset = rospy.get_param("/simple_localization/loc_node/measuring_offset")
        # print offset measurement related debug prints
        self.offset_debug_print = rospy.get_param("/simple_localization/loc_node/offset_debug_print")
        # the tower that is currently being measured offset for
        self.offset_tower_id = rospy.get_param("/simple_localization/loc_node/offset_tower_id")
        # the bot used for offset measurement
        self.offset_bot_id = rospy.get_param("/simple_localization/loc_node/offset_bot_id")
        ...
        self.current_measure_bot = self.offset_bot_id
        # the tower currently being measured for
        self.current_measure_tower = self.offset_tower_id
    ...
    def cb_tags(self, msg):
        ...
        @args:
            msg -> AprilTagDetectionArray: detected apriltag in each watchtower frame
        """

        # watchtower ID
        watchtower =msg.header.frame_id.split("/", 1)[0]
        frame_id = int(watchtower[-3:])
        for detection in msg.detections:
            # bot ID
            idx = self.tag2bot.get(detection.tag_id)
    ...
    def cb_markers(self, msg):
        ...
        if self.current_measure_bot != self.offset_bot_id:
            self.measure_bot_loc = None
            self.measure_bot_simple_loc = None
            # self.measure_bot_simple_loc = {}
            self.current_measure_bot = self.offset_bot_id
        ...
        for m in msg.markers:
            ...
            if m.ns == "watchtowers":
                ...
                if self.measuring_offset and idx == self.current_measure_tower:
                    self.pub_marker.publish(self.prep_marker(
        ...
        if self.measuring_offset:
            if self.current_measure_tower == self.offset_tower_id:
                if self.current_measure_tower in self.tower_offset:
